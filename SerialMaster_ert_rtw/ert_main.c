...
#include "SerialMaster_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(SerialMaster_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    SerialMaster_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(SerialMaster_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(SerialMaster_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(SerialMaster_M, 0);

  /* Initialize model */
  SerialMaster_initialize();
